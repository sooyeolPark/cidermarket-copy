...
<body>
  ...
  <section>
    ...
    <!--// 탭 메뉴 끝 -->
      ...
      <ul class="my_ctg">
        <li><a href="${pageContext.request.contextPath}/msgbox/store_receiver.cider">받은쪽지</a></li>
        <li class="active"><a href="${pageContext.request.contextPath}/msgbox/store_sender.cider">보낸쪽지</a></li>
      </ul>
                    ...
                    <c:forEach var="item" items="${output}" varStatus="status">
                    	<c:set var="num" value="${pageData.totalCount-pageData.listCount*(pageData.nowPage-1)-status.count+1}" />
   						 <tr>
      						...
      						<td class="msgbox_subject"><a href="#" class="clk_tr"><span class="user_id">${item.receiverNickname}</span> 님에게 <span class="msg_cont">[${item.subject}]</span> 상품에 대해 보낸 쪽지</a></td>
        ...
        <ul>
        	...
        	<c:choose>
                ...
                <c:when test="${pageData.prevPage > 0}">
                	<%-- 이동할 URL 생성 --%>
                    <c:url value="/msgbox/store_sender.cider" var="prevPageUrl">
            ...
            <%-- 페이지 번호 (시작 페이지 부터 끝 페이지까지 반복) --%>
               <c:forEach var="i" begin="${pageData.startPage}" end="${pageData.endPage}" varStatus="status">
                  <%-- 이동할 URL 생성 --%>
                  <c:url value="/msgbox/store_sender.cider" var="pageUrl">
        	...
        	<c:choose>
                ...
                <c:when test="${pageData.nextPage > 0}">
                	<%-- 이동할 URL 생성 --%>
                    <c:url value="/msgbox/store_sender.cider" var="nextPageUrl">
  ...
  <script type="text/javascript">
    $(function () {		
      ...
      $(".delete").click(function (e) { 		//삭제 버튼을 눌렀을때
        ...
        $.put("${pageContext.request.contextPath}/msgbox/store_sender",{
          	 "msgno": msgno
            }, function(json){
          	  if(json.rt=="OK"){
          		alert("삭제 완료되었습니다.");
          		//삭제 완료 후 목록 페이지로 이동
          		window.location = "${pageContext.request.contextPath}/msgbox/store_sender.cider";
          	  }
        });
      });
    });
