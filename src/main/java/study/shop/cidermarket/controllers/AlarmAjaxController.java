...
public class AlarmAjaxController {
	...
	/** 알람 페이지 */
	@RequestMapping(value = "/alarm.cider", method = RequestMethod.GET)
	public ModelAndView alarm(Model model,
		...
		try {
			// 전체 게시글 수 조회
			totalCount = alarmService.getAlarmCount(input);
		...
		return new ModelAndView("user/alarm");
	}
