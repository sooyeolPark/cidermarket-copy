...
public class CategoryServiceImpl implements CategoryService {
	...
	@Override
	public int updateProductCateno(Product input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.update("ProductMapper.updateCategoryItem", input);
		} catch (NullPointerException e) {
			log.error(e.getLocalizedMessage());
			throw new Exception("삭제된 데이터가 없습니다.");
		} catch (Exception e) {
	...
	public Files getFilesItem(Files input) throws Exception {
		Files result = null;
		try {
			result = sqlSession.selectOne("FilesMapper.selectAdmCategory", input);
			if(result == null) {
				throw new NullPointerException("result=null");
			}
		} catch (NullPointerException e) {
