...
@Slf4j
@Service
public class FilesCateServiceImpl implements FilesService {
	...
	public Files getFilesItem(Files input) throws Exception {
		Files result = null;
		try {
			result = sqlSession.selectOne("FilesCateMapper.selectItem", input);
			if(result == null) {
	...
	public List<Files> getFilesList(Files input) throws Exception {
		...
		try {
			result = sqlSession.selectList("FilesCateMapper.selectList", input);
			if(result == null) {
				throw new NullPointerException("result=null");
			}
		} catch (NullPointerException e) {
	...
	public int addFiles(Files input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.insert("FilesCateMapper.insertItem", input);
			if(result == 0) {
	...
	public int deleteFiles(Files input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.delete("FilesCateMapper.deleteItem", input);
			if(result == 0) {
				throw new NullPointerException("result=0");
			}
		} catch (NullPointerException e) {
