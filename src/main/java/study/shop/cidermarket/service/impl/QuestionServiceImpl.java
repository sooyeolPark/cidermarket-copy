...
@Slf4j
@Service
public class QuestionServiceImpl implements BbsService {
	...
	public Bbs getBbsItem(Bbs input) throws Exception {
		Bbs result = null;
		try {
			result = sqlSession.selectOne("QuestionMapper.selectItem", input);
			if(result == null) {
	...
	public List<Bbs> getBbsList(Bbs input) throws Exception {
		...
		try {
			result = sqlSession.selectList("QuestionMapper.selectList", input);
			if(result == null) {
				throw new NullPointerException("result=null");
			}
		} catch (NullPointerException e) {
	...
	public int getBbsCount(Bbs input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.selectOne("QuestionMapper.selectCountAll", input);
		} catch (Exception e) {
			log.error(e.getLocalizedMessage());
			throw new Exception("데이터 조회에 실패했습니다.");
		}
	...
	public int addBbs(Bbs input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.insert("QuestionMapper.insertItem", input);
			if(result == 0) {
	...
	public int editBbs(Bbs input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.update("QuestionMapper.updateItem", input);
			if(result == 0) {
				throw new NullPointerException("result=0");
			}
		} catch (NullPointerException e) {
	...
	public int deleteBbs(Bbs input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.delete("QuestionMapper.deleteItem", input);
			if(result == 0) {
				throw new NullPointerException("result=0");
			}
		} catch (NullPointerException e) {
