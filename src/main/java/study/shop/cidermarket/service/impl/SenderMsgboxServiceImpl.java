...
@Slf4j
@Service
public class SenderMsgboxServiceImpl implements MsgboxService {
	...
	public List<Msgbox> getMsgboxList(Msgbox input) throws Exception {
		...
		try {
			result = sqlSession.selectList("SenderMsgboxMapper.selectList", input);
			if(result == null) {
	...
	public int getMsgboxCount(Msgbox input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.selectOne("SenderMsgboxMapper.selectCountAll", input);
		} catch (Exception e) {
			log.error(e.getLocalizedMessage());
			throw new Exception("데이터 조회에 실패했습니다.");
		}
	...
	public int editMsgbox_save(Msgbox input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.update("SenderMsgboxMapper.saveMsg", input);
			if(result == 0) {
	...
	public int editMsgbox_delete(Msgbox input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.update("SenderMsgboxMapper.deleteMsg", input);
			if(result == 0) {
				throw new NullPointerException("result=0");
			}
		} catch (NullPointerException e) {
