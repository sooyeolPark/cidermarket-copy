...
@Slf4j
@Service
public class SStoreMsgboxServiceImpl implements MsgboxService {
	...
	public List<Msgbox> getMsgboxList(Msgbox input) throws Exception {
		...
		try {
			result = sqlSession.selectList("SStoreMsgboxMapper.selectList", input);
			if(result == null) {
	...
	public int getMsgboxCount(Msgbox input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.selectOne("SStoreMsgboxMapper.selectCountAll", input);
		} catch (Exception e) {
			log.error(e.getLocalizedMessage());
			throw new Exception("데이터 조회에 실패했습니다.");
		}
	...
	public int editMsgbox_delete(Msgbox input) throws Exception {
		int result = 0;
		try {
			result = sqlSession.update("SStoreMsgboxMapper.deleteMsg", input);
			if(result == 0) {
